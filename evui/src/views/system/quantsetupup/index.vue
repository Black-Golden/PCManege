...
<!--        <el-button @click="showEdit=true" type="primary" icon="el-icon-plus" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantsetupup:add')">添加-->
<!--        </el-button>-->
<!--        <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantsetupup:dall')">批量删除-->
<!--        </el-button>-->
      </div>
      <!-- 数据表格 -->
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column prop="rounds" label="分仓次数" sortable="custom" show-overflow-tooltip min-width="120"/>
...
<!--            <template slot-scope="{row}">-->
<!--&lt;!&ndash;              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:quantsetupup:edit')">修改</el-link>&ndash;&gt;-->
<!--              <el-popconfirm title="确定要删除此分仓设置吗？" @confirm="remove(row)" class="ele-action">-->
<!--                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:quantsetupup:delete')">删除</el-link>-->
<!--              </el-popconfirm>-->
<!--            </template>-->
<!--          </el-table-column>-->
        </template>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id?'修改分仓设置':'修改分仓设置'" :visible.sync="showEdit" width="450px"
      ...
      <el-form :model="editForm" ref="editForm" :rules="editRules" label-width="100px">
        ...
        <el-form-item label="分仓次数：" prop="rounds">
            <el-input-number v-model="editForm.rounds" controls-position="right" :min="0"
                              placeholder="请输入分仓次数" class="ele-fluid ele-text-left"/>
        </el-form-item>
...
export default {
  name: "SysQuantSetupUp",
  data() {
    return {
      table: {url: '/quantsetupup/index', where: {}},  // 表格配置
      choose: [],  // 表格选中数据
      ...
      editRules: {  // 表单验证规则
        ...
        rounds: [
          {required: true, message: '请输入分仓次数', trigger: 'blur'}
        ],
...
computed: {
  ...
  methods: {
     ...
     */
    edit(row) {
      this.$http.get('/quantsetupup/info?id=' + row.id).then(res => {
        if (res.data.code === 0) {
    ...
    save() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/quantsetupup/edit', this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      ...
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的分仓设置吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.post('/quantsetupup/delete', {id: ids}).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
        ...
        const loading = this.$loading({lock: true});
        this.$http.post('/quantsetupup/delete', {id:row.id}).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({type: 'success', message: res.data.msg});
            this.$refs.table.reload();
