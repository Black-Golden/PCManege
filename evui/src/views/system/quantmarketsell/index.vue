...
<!--        <el-button @click="showEdit=true" type="primary" icon="el-icon-plus" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantmarketsell:add')">添加-->
<!--        </el-button>-->
<!--        <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantmarketsell:dall')">批量删除-->
<!--        </el-button>-->
      </div>
      <!-- 数据表格 -->
...
<!--            <template slot-scope="{row}">-->
<!--&lt;!&ndash;              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:quantmarketsell:edit')">修改</el-link>&ndash;&gt;-->
<!--              <el-popconfirm title="确定要删除此交易买入吗？" @confirm="remove(row)" class="ele-action">-->
<!--                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:quantmarketsell:delete')">删除</el-link>-->
<!--              </el-popconfirm>-->
<!--            </template>-->
<!--          </el-table-column>-->
        </template>
...
export default {
  name: "SysQuantMarketSell",
  data() {
    return {
      table: {url: '/quantmarketsell/index', where: {}},  // 表格配置
      choose: [],  // 表格选中数据
...
computed: {
  ...
  methods: {
     ...
     */
    edit(row) {
      this.$http.get('/quantmarketsell/info?id=' + row.id).then(res => {
        if (res.data.code === 0) {
    ...
    save() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/quantmarketsell/edit', this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      ...
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的交易买入吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.post('/quantmarketsell/delete', {id: ids}).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
        ...
        const loading = this.$loading({lock: true});
        this.$http.post('/quantmarketsell/delete', {id:row.id}).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({type: 'success', message: res.data.msg});
            this.$refs.table.reload();
    ...
    setIsDeal(row) {
        ...
        this.$http.post("/quantmarketsell/setIsDeal", params).then(res => {
            loading.close();
            if (res.data.code === 0) {
                this.$message({type: 'success', message: res.data.msg});
            } else {
    ...
    setIsBonus(row) {
        const loading = this.$loading({lock: true});
        let params = Object.assign({}, row);
        this.$http.post("/quantmarketsell/setIsBonus", params).then(res => {
            loading.close();
            if (res.data.code === 0) {
                this.$message({type: 'success', message: res.data.msg});
            } else {
