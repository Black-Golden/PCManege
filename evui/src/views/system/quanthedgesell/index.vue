  ...
  <div class="ele-body">
    <el-card shadow="never">
      ...
      <div class="ele-table-tool ele-table-tool-default">
        <el-button @click="showEdit=true" type="primary" icon="el-icon-plus" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quanthedgesell:add')">添加
        </el-button>
        <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quanthedgesell:dall')">批量删除
        </el-button>
      </div>
      <!-- 数据表格 -->
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column prop="user_id" label="管理用户" sortable="custom" show-overflow-tooltip min-width="120"/>
          ...
          <el-table-column prop="qb_source" label="卖出数量" sortable="custom" show-overflow-tooltip min-width="120"/>
                                
          <el-table-column prop="price" label="卖出行情价" sortable="custom" show-overflow-tooltip min-width="120"/>
                                
          <el-table-column prop="qb_to" label="USDT数量" sortable="custom" show-overflow-tooltip min-width="120"/>
          ...
          <el-table-column prop="is_bonus" label="是否结算" sortable="custom" :resizable="false" min-width="120">
              <template slot-scope="{row}">
                  <el-switch v-model="row.is_bonus" @change="setIsBonus(row)" :active-value="1" :inactive-value="2"/>
              </template>
          </el-table-column>
                                
          <el-table-column prop="times" label="卖出次数" sortable="custom" show-overflow-tooltip min-width="120"/>
          ...
          <el-table-column label="操作" width="130px" align="center" :resizable="false"  fixed="right">
            <template slot-scope="{row}">
              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:quanthedgesell:edit')">修改</el-link>
              <el-popconfirm title="确定要删除此对冲交易卖出吗？" @confirm="remove(row)" class="ele-action">
                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:quanthedgesell:delete')">删除</el-link>
              </el-popconfirm>
            </template>
          </el-table-column>
        </template>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id?'修改对冲交易卖出':'修改对冲交易卖出'" :visible.sync="showEdit" width="450px"
      ...
      <el-form :model="editForm" ref="editForm" :rules="editRules" label-width="100px">
        ...
        <el-form-item label="卖出数量：" prop="qb_source">
            <el-input-number v-model="editForm.qb_source" controls-position="right" :min="0"
                              placeholder="请输入卖出数量" class="ele-fluid ele-text-left"/>
        </el-form-item>
                                                      
        <el-form-item label="卖出行情价：" prop="price">
            <el-input-number v-model="editForm.price" controls-position="right" :min="0"
                              placeholder="请输入卖出行情价" class="ele-fluid ele-text-left"/>
        </el-form-item>
                                                      
        <el-form-item label="USDT数量：" prop="qb_to">
        ...
        <el-form-item label="是否结算：" prop="is_bonus">
          <el-switch
        ...
        <el-form-item label="卖出次数：" prop="times">
            <el-input-number v-model="editForm.times" controls-position="right" :min="0"
                              placeholder="请输入卖出次数" class="ele-fluid ele-text-left"/>
        </el-form-item>
...
export default {
  name: "SysQuantHedgeSell",
  data() {
    return {
      table: {url: '/quanthedgesell/index', where: {}},  // 表格配置
      choose: [],  // 表格选中数据
      ...
      editRules: {  // 表单验证规则
        ...
        times: [
          {required: true, message: '请输入卖出次数', trigger: 'blur'}
        ],
...
computed: {
  ...
  methods: {
     ...
     */
    edit(row) {
      this.$http.get('/quanthedgesell/info?id=' + row.id).then(res => {
        if (res.data.code === 0) {
    ...
    save() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/quanthedgesell/edit', this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      ...
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的对冲交易卖出吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.post('/quanthedgesell/delete', {id: ids}).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
        ...
        const loading = this.$loading({lock: true});
        this.$http.post('/quanthedgesell/delete', {id:row.id}).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({type: 'success', message: res.data.msg});
            this.$refs.table.reload();
    ...
    setIsDeal(row) {
        ...
        this.$http.post("/quanthedgesell/setIsDeal", params).then(res => {
            loading.close();
            if (res.data.code === 0) {
                this.$message({type: 'success', message: res.data.msg});
            } else {
    ...
    setIsBonus(row) {
        const loading = this.$loading({lock: true});
        let params = Object.assign({}, row);
        this.$http.post("/quanthedgesell/setIsBonus", params).then(res => {
            loading.close();
            if (res.data.code === 0) {
                this.$message({type: 'success', message: res.data.msg});
            } else {
