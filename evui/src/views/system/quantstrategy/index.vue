  ...
  <div class="ele-body">
    <el-card shadow="never">
        ...
        <el-row :gutter="15">
            ...
            <el-col :md="6" :sm="12">
              <el-form-item label="代币类型:">
                <el-input v-model="table.where.title" placeholder="请输入代币类型" clearable/>
              </el-form-item>
            </el-col>
              <el-col :md="6" :sm="12">
      ...
      <div class="ele-table-tool ele-table-tool-default">
        <el-button @click="showEdit=true" type="primary" icon="el-icon-plus" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantstrategy:add')">添加
        </el-button>
        <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantstrategy:dall')">批量删除
        </el-button>
      </div>
      <!-- 数据表格 -->
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column prop="title" label="代币类型" sortable="custom" show-overflow-tooltip min-width="120"/>
          ...
          <el-table-column label="操作" width="130px" align="center" :resizable="false"  fixed="right">
            <template slot-scope="{row}">
              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:quantstrategy:edit')">修改</el-link>
              <el-popconfirm title="确定要删除此交易策略吗？" @confirm="remove(row)" class="ele-action">
                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:quantstrategy:delete')">删除</el-link>
              </el-popconfirm>
            </template>
          </el-table-column>
        </template>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id?'修改交易策略':'修改交易策略'" :visible.sync="showEdit" width="450px"
               @closed="editForm={}" :destroy-on-close="true" :lock-scroll="false">
      <el-form :model="editForm" ref="editForm" :rules="editRules" label-width="100px">
                          
        <el-form-item label="代币类型：" prop="title">
            <el-input v-model="editForm.title" placeholder="请输入代币类型" clearable/>
        </el-form-item>
...
export default {
  name: "SysQuantStrategy",
  data() {
    return {
      table: {url: '/quantstrategy/index', where: {}},  // 表格配置
      choose: [],  // 表格选中数据
...
computed: {
  ...
  methods: {
     ...
     */
    edit(row) {
      this.$http.get('/quantstrategy/info?id=' + row.id).then(res => {
        if (res.data.code === 0) {
    ...
    save() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/quantstrategy/edit', this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      ...
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的交易策略吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.post('/quantstrategy/delete', {id: ids}).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
        ...
        const loading = this.$loading({lock: true});
        this.$http.post('/quantstrategy/delete', {id:row.id}).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({type: 'success', message: res.data.msg});
            this.$refs.table.reload();
