...
<!--        <el-button @click="showEdit=true" type="primary" icon="el-icon-plus" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantmarketitem:add')">添加-->
<!--        </el-button>-->
<!--        <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small" v-if="permission.includes('sys:quantmarketitem:dall')">批量删除-->
<!--        </el-button>-->
      </div>
      <!-- 数据表格 -->
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column prop="price_min" label="购买行情价最小金额" sortable="custom" show-overflow-tooltip min-width="120"/>
                                
          <el-table-column prop="price_max" label="购买行情价最大金额" sortable="custom" show-overflow-tooltip min-width="120"/>
          ...
          <el-table-column prop="price_close" label="强平价格（针对合约使用）" sortable="custom" show-overflow-tooltip min-width="120"/>
...
<!--            <template slot-scope="{row}">-->
<!--&lt;!&ndash;              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:quantmarketitem:edit')">修改</el-link>&ndash;&gt;-->
<!--              <el-popconfirm title="确定要删除此交易详情吗？" @confirm="remove(row)" class="ele-action">-->
<!--                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:quantmarketitem:delete')">删除</el-link>-->
<!--              </el-popconfirm>-->
<!--            </template>-->
<!--          </el-table-column>-->
        </template>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id?'修改交易详情':'修改交易详情'" :visible.sync="showEdit" width="450px"
      ...
      <el-form :model="editForm" ref="editForm" :rules="editRules" label-width="100px">
        ...
        <el-form-item label="购买行情价最小金额：" prop="price_min">
            <el-input-number v-model="editForm.price_min" controls-position="right" :min="0"
                              placeholder="请输入购买行情价最小金额" class="ele-fluid ele-text-left"/>
        </el-form-item>
                                                      
        <el-form-item label="购买行情价最大金额：" prop="price_max">
            <el-input-number v-model="editForm.price_max" controls-position="right" :min="0"
                              placeholder="请输入购买行情价最大金额" class="ele-fluid ele-text-left"/>
        </el-form-item>
        ...
        <el-form-item label="强平价格（针对合约使用）：" prop="price_close">
            <el-input-number v-model="editForm.price_close" controls-position="right" :min="0"
                              placeholder="请输入强平价格（针对合约使用）" class="ele-fluid ele-text-left"/>
        </el-form-item>
...
export default {
  name: "SysQuantMarketItem",
  data() {
    return {
      table: {url: '/quantmarketitem/index', where: {}},  // 表格配置
      choose: [],  // 表格选中数据
      ...
      editRules: {  // 表单验证规则
        ...
        price_min: [
          {required: true, message: '请输入购买行情价最小金额', trigger: 'blur'}
        ],
                          
        price_max: [
          {required: true, message: '请输入购买行情价最大金额', trigger: 'blur'}
        ],
        ...
        price_close: [
          {required: true, message: '请输入强平价格（针对合约使用）', trigger: 'blur'}
        ],
...
computed: {
  ...
  methods: {
     ...
     */
    edit(row) {
      this.$http.get('/quantmarketitem/info?id=' + row.id).then(res => {
        if (res.data.code === 0) {
    ...
    save() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/quantmarketitem/edit', this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      ...
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的交易详情吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.post('/quantmarketitem/delete', {id: ids}).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
        ...
        const loading = this.$loading({lock: true});
        this.$http.post('/quantmarketitem/delete', {id:row.id}).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({type: 'success', message: res.data.msg});
            this.$refs.table.reload();
    ...
    setIsSale(row) {
        ...
        this.$http.post("/quantmarketitem/setIsSale", params).then(res => {
            loading.close();
            if (res.data.code === 0) {
                this.$message({type: 'success', message: res.data.msg});
            } else {
