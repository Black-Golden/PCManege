<template>
  <div class="ele-body">
    <el-card shadow="never">
        ...
        <el-row :gutter="15">
            ...
            <el-col :md="6" :sm="12">
              <el-form-item label="分组名称:">
                <el-input v-model="table.where.name" placeholder="请输入分组名称" clearable/>
              </el-form-item>
            </el-col>
              <el-col :md="6" :sm="12">
      ...
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column prop="name" label="分组名称" sortable="custom" show-overflow-tooltip min-width="120"/>
          ...
          <el-table-column label="操作" width="130px" align="center" :resizable="false"  fixed="right">
            <template slot-scope="{row}">
              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:config:edit')">修改</el-link>
              <el-popconfirm title="确定要删除此配置分组吗？" @confirm="remove(row)" class="ele-action">
                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:config:delete')">删除</el-link>
              </el-popconfirm>
            </template>
          </el-table-column>
        </template>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id?'修改配置分组':'修改配置分组'" :visible.sync="showEdit" width="450px"
               @closed="editForm={}" :destroy-on-close="true" :lock-scroll="false">
      <el-form :model="editForm" ref="editForm" :rules="editRules" label-width="100px">
                          
        <el-form-item label="分组名称：" prop="name">
            <el-input v-model="editForm.name" placeholder="请输入分组名称" clearable/>
        </el-form-item>
                                                  
        <el-form-item label="排序：" prop="sort">
            <el-input-number v-model="editForm.sort" controls-position="right" :min="0"
                              placeholder="请输入排序" class="ele-fluid ele-text-left"/>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      table: {url: '/config/index', where: {}},  // 表格配置
      choose: [],  // 表格选中数据
